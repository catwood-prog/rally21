-- Found while writing the live test suite: calling get_my_blueprint()
-- twice within the same transaction failed because "on commit drop"
-- only drops at actual COMMIT, not at function return — PostgREST gives
-- each request its own transaction so this never bit production, but a
-- temp table should be reentrant regardless. Drop it explicitly first.
