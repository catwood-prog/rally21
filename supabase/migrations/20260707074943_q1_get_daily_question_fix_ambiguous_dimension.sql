-- Fixes Postgres error 42702 ("column reference \"dimension\" is ambiguous").
-- get_daily_question's own `returns table (..., dimension text, ...)`
-- declaration creates an implicit OUT-parameter `dimension` in scope for
-- the whole function body. The dimension-rest-trigger CTE's aggregate
-- referenced the CTE's own `dimension` column unqualified, which Postgres
-- couldn't disambiguate from the OUT parameter. Qualifies both
-- references to the CTE alias explicitly.
