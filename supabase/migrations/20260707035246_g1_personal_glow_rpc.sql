-- G1: personal glow (Rally21-Glow-Spec.md §1-2, §6). Server-computed
-- streak: consecutive local days with >=1 self completion in ANY circle;
-- covered days hold (don't break, don't increment) up to a shelter
-- capacity that grows with lifetime milestones (21/50/100). A missed,
-- unheld day opens a 48h ember window before cooling to zero.
...
begin
  ...
  -- Lifetime max glow ever reached, for shelter capacity (spec: derive
  -- from history, don't store). A consecutive-day run bridging any
  -- covered day (no capacity limit applied retroactively here — this is
  -- just "how high has the self-day count climbed within one unbroken
  -- run", not a faithful replay of past capacity mechanics).
  ...
  -- Walk forward day by day from first-ever activity through yesterday,
  -- tracking the live streak + monthly hold spend (chronological within
  -- the month, per spec). Today is handled separately below — a day in
  -- progress can't yet be a "miss".
  v_cursor := v_start;
  while v_cursor <= v_yesterday loop
    ...
    if v_is_self then
      ...
      null; -- unchanged, chain continues
    else
  ...
  if v_last_break_date is not distinct from v_yesterday then
    -- yesterday broke the chain: embers/cold unless today rekindles it
    if v_today_self then
      v_glow := v_glow_before_last_break + 1;
      v_state := 'glowing';
      v_ember_deadline := null;
    else
      v_ember_deadline := ((v_yesterday + 2)::timestamp at time zone v_tz);
      if now() < v_ember_deadline then
        v_state := 'embers';
    ...
    -- chain alive through yesterday (or no history at all) — today just
    -- extends it, holds it, or grace-continues it unresolved.
    if v_today_self then
