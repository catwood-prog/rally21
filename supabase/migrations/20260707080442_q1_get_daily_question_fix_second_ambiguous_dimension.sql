-- Second instance of the same class of bug (42702): get_daily_question's
-- own `returns table (..., dimension text, ...)` OUT parameter shadows
-- any bare `dimension` identifier in the function body, including
-- ON CONFLICT target lists (which cannot be table-qualified in Postgres).
-- Fixes the question_dimension_rests upsert by moving it into a helper
-- function with no colliding parameter name, called via `perform`.

create or replace function public._rest_question_dimension(p_user uuid, p_dimension text, p_rested_until date)
returns void
language plpgsql
...
begin
  ...
  values (p_user, p_dimension, p_rested_until, now())
...
revoke all on function public._rest_question_dimension(uuid, text, date) from anon, public;
grant execute on function public._rest_question_dimension(uuid, text, date) to authenticated;
